...
export default function Leaves() {
  ...
  useEffect(() => {
    async function fetchHolidays() {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/holidays?year=${new Date().getFullYear()}`, {
          headers: {
  ...
  const handleRemoveHoliday = async (date: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/holidays/${holiday._id}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  useEffect(() => {
    async function fetchLeaves() {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/leaveRequests`, {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
          },
